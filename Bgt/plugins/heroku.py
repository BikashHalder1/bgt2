...
from Bgt import app
from Bgt.misc import HAPP, XCB, SUDOERS
from Bgt.utils.pastebin import Javabin
from git.exc import GitCommandError, InvalidGitRepositoryError
from Bgt.utils.database import get_active_chats, remove_active_chat, remove_active_video_chat
...
async def vardel_(client, message):
    ...
    if await is_heroku():
        ...
        if not output[0]:
            ...
            await message.reply_text("{0} ᴅᴇʟᴇᴛᴇᴅ.".format(check_var))
            os.system(f"kill -9 {os.getpid()} && Python3 -m Bgt")
...
@app.on_message(filters.command(["usage"]) & filters.user(OWNER_ID) & ~filters.forwarded)
