...
from Bgt.utils import get_readable_time
from Bgt.utils.database import add_banned_user, get_banned_count, get_banned_users, get_served_chats, is_banned_user, remove_banned_user
...
async def gbannuser(client, message: Message):
    ...
    if is_gbanned:
        return await message.reply_text("{0} ɪs ᴀʟʀᴇᴀᴅʏ **ɢʙᴀɴɴᴇᴅ** ғʀᴏᴍ ᴛʜᴇ ʙᴏᴛ.".format(mention))
    ...
    mystic = await message.reply_text("**ɢʟᴏʙᴀʟ ʙᴀɴɴɪɴɢ {0}**\n\nᴇxᴩᴇᴄᴛᴇᴅ ᴛɪᴍᴇ : {1}.".format(mention, time_expected))
    number_of_chats = 0
    ...
    await message.reply_text("**ɢʙᴀɴɴᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ**\n\nʙᴀɴɴᴇᴅ **{0}** ғʀᴏᴍ **{1}** ᴄʜᴀᴛs.".format(mention, number_of_chats))
    await mystic.delete()


@app.on_message(filters.command(["ungban"]) & SUDOERS)
async def ungbannuser(client, message: Message):
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
        mention = message.reply_to_message.from_user.mention
    else:
    ...
    is_gbanned = await is_banned_user(user_id)
    if not is_gbanned:
        return await message.reply_text("{0} ɪꜱ ɴᴏᴛ **ɢʙᴀɴɴᴇᴅ** ʏᴇᴛ ꜰʀᴏᴍ ᴛʜᴇ ʙᴏᴛ.".format(mention))
    ...
    time_expected = len(served_chats)
    time_expected = get_readable_time(time_expected)
    mystic = await message.reply_text("**ᴜɴɢʙᴀɴɴɪɴɢ {0}**\n\nᴇxᴩᴇᴄᴛᴇᴅ ᴛɪᴍᴇ : {1}.".format(mention, time_expected))
    number_of_chats = 0
    for chat_id in served_chats:
        try:
    ...
    await message.reply_text("**ᴜɴɢʙᴀɴɴᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ**\n\nᴜɴʙᴀɴɴᴇᴅ **{0}** ɪɴ **{1}** ᴄʜᴀᴛs.".format(mention, number_of_chats))
    await mystic.delete()
