...
from Bgt import app
from Bgt.utils import extract_user, int_to_alpha
from Bgt.utils.database import delete_authuser, get_authuser, get_authuser_names, save_authuser
from Bgt.utils.decorators import AdminActual
from Bgt.utils import close_keyboard
...
async def auth(client, message: Message):
    ...
    count = len(_check)
    if int(count) == 25:
        return await message.reply_text("» ʏᴏᴜ ᴄᴀɴ ᴏɴʟʏ ʜᴀᴠᴇ 25 ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴜsᴇʀs ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ !")
    if token not in _check:
        ...
        return await message.reply_text("» ᴀᴅᴅᴇᴅ {0} ᴛᴏ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴜsᴇʀs ʟɪsᴛ !".format(user.mention))
    else:
        return await message.reply_text("{0} ɪs ᴀʟʀᴇᴀᴅʏ ɪɴ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴜsᴇʀs ʟɪsᴛ !".format(user.mention))


@app.on_message(filters.command(["unauth", "delauth"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def unauthusers(client, message: Message):
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text("» ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴜsᴇʀ's ᴍᴇssᴀɢᴇ ᴏʀ ɢɪᴠᴇ ᴜsᴇʀɴᴀᴍᴇ/ᴜsᴇʀ ɪᴅ !")
    user = await extract_user(message)
    ...
    if get:
        if user.id in get:
            get.remove(user.id)
    if deleted:
        return await message.reply_text("» ʀᴇᴍᴏᴠᴇᴅ {0} ғʀᴏᴍ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴜsᴇʀs ʟɪsᴛ !".format(user.mention))
    else:
        return await message.reply_text("{0} ɪs ɴᴏᴛ ɪɴ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴜsᴇʀs ʟɪsᴛ !".format(user.mention))
