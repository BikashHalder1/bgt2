...
from Bgt.utils.database import add_gban_user, remove_gban_user
...
async def useradd(client, message: Message):
    ...
    if user.id in BANNED_USERS:
        return await message.reply_text("{0} ɪs ᴀʟʀᴇᴀᴅʏ ʙʟᴏᴄᴋᴇᴅ ꜰʀᴏᴍ ᴛʜᴇ ʙᴏᴛ".format(message.reply_to_message.from_user.mention))
    elif user.id in SUDOERS:
        return await message.reply_text("ɴᴏ, ᴛʜɪꜱ ᴜꜱᴇʀ ɪꜱ ᴀ ꜱᴜᴅᴏ ᴜꜱᴇʀ ᴏꜰ ᴛʜɪꜱ ʙᴏᴛ")
    await add_gban_user(user.id)
    BANNED_USERS.add(user.id)
    await message.reply_text("ᴀᴅᴅᴇᴅ **{0}** ᴛᴏ ʙʟᴏᴄᴋ ʟɪsᴛ ᴏꜰ ʙᴏᴛ. ᴜsᴇʀ ᴡᴏɴ'ᴛ ʙᴇ ᴀʙʟᴇ ᴛᴏ ᴜsᴇ ʙᴏᴛ ɴᴏᴡ ᴜɴᴅᴇʀ ᴀɴʏ ᴄᴏɴᴅɪᴛɪᴏɴ.\n\nᴄʜᴇᴄᴋ ʙʟᴏᴄᴋᴇᴅ ᴜsᴇʀs: /blockedusers".format(user.mention))
...
async def userdel(client, message: Message):
    ...
    await message.reply_text("ʀᴇᴍᴏᴠᴇᴅ ᴜsᴇʀ ꜰʀᴏᴍ ᴛʜᴇ ʙʟᴏᴄᴋ ʟɪsᴛ. ᴜsᴇʀ ᴡɪʟʟ ʙᴇ ᴀʙʟᴇ ᴛᴏ ᴜsᴇ ʙᴏᴛ ɴᴏᴡ.")
