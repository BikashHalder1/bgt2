...
from pyrogram.types import ChatPermissions, ChatPrivileges, Message, InlineKeyboardButton, InlineKeyboardMarkup, CallbackQuery
from Bgt import app
from Bgt.utils.database import add_warn, get_warn, remove_warns, save_filter
from Bgt.utils.admins import list_admins, adminsOnly, ownerOnly, member_permissions
from Bgt.utils import extract_user, extract_user_and_reason, time_converter, int_to_alpha
...
⊱ /setting - ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ sᴇᴛ ғᴇᴀᴛᴜʀᴇs ᴡʜɪᴄʜ
...
async def purgeFunc(_, message: Message):
    try:
        ...
        await message.reply_text("ɪ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴇɴᴏᴜɢʜ ᴘᴇʀᴍɪssɪᴏɴs !\n\n**ᴘᴇʀᴍɪssɪᴏɴ** :\n`can_delete_messages`")
...
async def unban_func(_, message: Message):
    ...
    try:
        ...
        if len(message.command) == 2:
            ...
            return await message.reply_text("ᴘʀᴏᴠɪᴅᴇ ᴀ ᴜsᴇʀɴᴀᴍᴇ ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴜsᴇʀ's ᴍᴇssᴀɢᴇ ᴛᴏ ᴜɴʙᴀɴ !")
...
@app.on_message(filters.command("banda") & ~filters.private)
@adminsOnly("can_restrict_members")
...
async def warn_user(_, message: Message):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                   InlineKeyboardButton(text="ʀᴇᴍᴏᴠᴇ ᴡᴀʀɴ", callback_data=f"unwarn_{user_id}")
                ]
            ]
        )
        if warns:
        ...
        if warns >= 2:
            await message.chat.ban_member(user_id)
            await message.reply_text(f"ɴᴜᴍʙᴇʀ ᴏғ ᴡᴀʀɴs ᴏғ {mention} ᴇxᴄᴇᴇᴅᴇᴅ !\nʙᴀɴɴᴇᴅ !")
...
async def remove_warnings(_, message: Message):
    ...
    try:
        if not message.reply_to_message:
            return await message.reply_text("ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴛᴏ ʀᴇᴍᴏᴠᴇ ᴀ ᴜsᴇʀ's ᴡᴀʀɴɪɴɢ !")
...
async def remove_warning(_, cq: CallbackQuery):
    ...
    if permission not in permissions:
        return await cq.answer("ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴇɴᴏᴜɢʜ ᴘᴇʀᴍɪssɪᴏɴs ᴛᴏ ᴘᴇʀғᴏʀᴍ ᴛʜɪs ᴀᴄᴛɪᴏɴ !", show_alert=True)
