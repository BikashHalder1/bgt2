...
async def lrsearch(client, message: Message):
    ...
    if S is None:
        return await m.edit_text("❌ ꜰᴀɪʟᴇᴅ ᴛᴏ ꜰᴇᴛᴄʜ ʟʏʀɪᴄs.\n\n» **ᴛʀɪᴇᴅ sᴇᴀʀᴄʜɪɴɢ ꜰᴏʀ:** {0}".format(title), disable_web_page_preview=True)
    lyric = S.lyrics
    if "Embed" in lyric:
    ...
    with open(ran_hash, "w") as lyr:
        lyr.write(lyric)
    try:
        await message.reply_document(ran_hash, caption="ʜᴇʀᴇ ɪs ʏᴏᴜʀ ʟʏʀɪᴄs\n\n⊱ **Tɪᴛʟᴇ :** `{}`".format(title), file_name=ran_hash)
        await m.delete()
